...
// Grab the dataset (use 30 day data for all earthquake occurences)
var linkgeo = "https://earthquake.usgs.gov/earthquakes/feed/v1.0/summary/all_month.geojson";

// Create binding to json file
d3.json(linkgeo, function(response) {

    // list to hold circle markers
    var listlayer = [];
    // Looped through data into tree, and then into geometry with coordinates
    ...
    d3.json(linkbound, function(boundata) {
        ...
        // Create list for tectonic plate bounderies, used as overlayMap
        var scndlayer = [];
        ...
        // Controls & Settings
        // Create Layer Group for all points pushed to listlayer
        var earthqks = new L.layerGroup(listlayer);
        var pltbounds = new L.layerGroup(scndlayer);

        // Title layer(s) to map
        ...
        var myMap = L.map("map", {
            ...
            zoom: 3.4,
            layers: [strets, earthqks, pltbounds]
        });

        // Set control, activate myMap
        L.control.layers(baseLayers, overlayMaps).addTo(myMap);

        //Customize and create legend control
        ...
        legend.onAdd = function(myMap) {
            ...
            limits.forEach(function(limit, index) {
                legenddisp.push("<li style=\"list-style-type:none;background-color:" 
                + colors[index] + "\">" + "<h6>" + labels[index] +"</h6></li>");
            });
            div.innerHTML += "<ul>" + legenddisp.join("") + "</ul>";
            return div;
        };
        legend.addTo(myMap);
    });
